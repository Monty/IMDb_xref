...
- **`saveFilmography.sh`** — Generates GitHub-flavored `.md` file alongside `.json`. Groups roles by job, consolidates characters per title, name is H1 link to IMDb (strips "(I)" from display), sorts by last year of range. Filters noise (self, performer, crew roles).
